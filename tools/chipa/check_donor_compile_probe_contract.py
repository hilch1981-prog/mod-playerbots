...
def main() -> int:
    ...
    require(text, "option(CHIPA_PLAYERBOT_DONOR_COMPILE_PROBE", "opt-in probe option")
    require(text, '  OFF)', "default-OFF probe state")
    require(text, "if(CHIPA_PLAYERBOT_DONOR_COMPILE_PROBE)", "probe guard")
    require(
        text,
    ...
    guarded = text.split("if(CHIPA_PLAYERBOT_DONOR_COMPILE_PROBE)", 1)[1]
    ...
    normal_prefix = text.split("if(CHIPA_PLAYERBOT_DONOR_COMPILE_PROBE)", 1)[0]
